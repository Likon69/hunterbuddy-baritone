...
public final class Settings {
    ...
    /**
     * When a standing takeoff spot turns out to be walled in on every side (a crevice or hole
     * narrower than the takeoff needs), pillar up out of it with a block from the hotbar before
     * jumping, instead of taking off there anyway and falling right back in, or giving up. Needs
     * a throwaway block in {@link #acceptableThrowawayItems} and {@link #allowPlace}.
     */
    ...
    /**
     * Land on any block whose top face is solid (full collision shape or a sturdy up face),
     * instead of only netherrack, gravel and (if {@link #elytraAllowLandOnNetherFortress}) nether
     * brick. Obsidian, basalt, blackstone and soul soil all qualify; magma, fluids and bastion
     * blocks never do regardless of this setting. Off restores the old, narrower list.
     */
    public final Setting<Boolean> elytraLandOnAnySolid = new Setting<>(true);

    /**
     * How close a piglin brute may be to a candidate landing spot before it is treated as inside a
     * bastion and skipped, the same way a bastion block within the spot's own footprint is.
     */
    public final Setting<Integer> elytraLandingBastionRadius = new Setting<>(48);
